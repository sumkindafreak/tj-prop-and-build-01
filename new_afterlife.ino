...
#define HEAT_DLY  10000  // How long should the "overheat" ramp up last?
...
void safety1_read() {
  if (!digitalRead(SAFETY_ONE) && (startwand == true)) {
    if (wand_on)
      mp3.playTrack(11);
    WANDledstate2();
    safety2_read ();
    ...
    **/      // ******************* lets power up the pack! ******************* //
    ...
    if (!digitalRead(STARTPACK_SWITCH) ) {
      ...
      if (PowerBooted == false) {
        PowerBooted = !PowerBooted;   //true
        startpack = true;              // true
        mp3.playTrack(8);

        // need this track  ( 8 ) to play for 12 seconds
        // before moving onto the next stage but not interfering with the bargraph charging or charging sequence from the wand
      }
 /******************************************** HERE IS THE MAGIC/IDLE/ begin reading STAGE*******************************************************************
      ...
      */    
      ...
      if (!digitalRead(STARTWAND_SWITCH)&& (startpack == true)) {
        if (PowerBooted) {
          startwand = true;     // true
          WANDledstate2();      // turns wand on
          mp3.playTrack(1);     // wanbd on sound
          safety1_read();        // read the first safety switch
          wand_on = true;         //true
        }
        else
          mp3.playTrack(1);      //false
      ...
      if (digitalRead(STARTPACK_SWITCH) && (poweroff == false) ) {
        startpack = false;                  //false
        PowerBooted = !PowerBooted;         //false
        Console.println("POWERED DOWN");
        cyclotron.update();
        Serial.println("POWER Off");  //off
        mp3.playTrack(9);
  ...
  /******************************************** HERE IS THE MAGIC/IDLE/WORKING STAGE*******************************************************************
